...
def search_all_keywords(word_list, keywords):
    retval = {}
    for keyword in keywords:
        while True:
            try:
                ...
                if index > 0:
                    retval[keyword] = retval.get(keyword, 0) + 1
...
class Command:
    ...
    def get_word_windows(self, keyword, keywords, original_words, window_length):
        ...
        while True:
            ...
            window_1_keywords_found = search_all_keywords(window_1, keywords)
            window_2_keywords_found = search_all_keywords(window_2, keywords)

            count = 0
            found_terms = []

            for term, _count in window_1_keywords_found.items():
                count += _count
                found_terms.append(term)

            for term, _count in window_2_keywords_found.items():
                count += _count
                found_terms.append(term)

            pair = (' '.join(window_1), ' '.join(window_2), count, found_terms)
            pairs.append(pair)
    ...
    def run(self, limit):
        ...
        with open(ids_file, 'r') as f:
            while True:
                ...
                result = result_by_source.get(source, None)
                if result is None:
                    result = {}
                    for keyword in keywords:
                        result[keyword] = []
                ...
                for keyword, extracted_rows in result.items():
                    pairs = self.get_word_windows(keyword, keywords, original_words, 20)
                    for before20, after20, count, found_terms in pairs:
                        extracted_rows.append((before20, after20, count, found_terms, metadata))

                bar.next()
        bar.finish()

        output_file_template = 'files/xlsx/word_metadata_windows_list_output_{}.csv'

        for source, result in result_by_source.items():
            output_file = output_file_template.format(source)
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for keyword, extracted_rows in result.items():
                    ...
                    for before20, after20, count, found_terms, metadata in extracted_rows:
                        f.write(keyword)
                        f.write('\t')
                        f.write(before20)
                        f.write('\t')
